...
noun_training_step = 1000
verb_training_step = 500
...
#####################################################
####Plot the histogram of the SimLex-999 pairs#######
#####################################################
store_histo = []
with open("sense_spectrum_pair", 'r') as f: #read data
    for line in f:
        temp_histo = []
        c = line.split(',')
        temp_histo.append(c[0])
        temp_histo.append(c[1])
        k = re.findall(r"[-+]?\d*\.\d+|\d+", line)
        temp_histo.append(k)
        store_histo.append(k)#store data

calcu_ = []   
for i in range(len(store_histo)):
    t_c = [] #temp_cell
    t_c.append(float(store_histo[i][2]))#alpha
    t_c.append(float(store_histo[i][3]))#beta
    t_c.append(float(store_histo[i][4]))#gamma
    t_c.append(float(store_histo[i][5]))#alpha_hat
    t_c.append(float(store_histo[i][7]))#beta_hat
    t_c.append(float(store_histo[i][9]))#gamma_hat
    t_ = (np.abs(t_c[0]-t_c[3])+np.abs(t_c[1]-t_c[4])+np.abs(t_c[2]-t_c[5]))/(t_c[0]+t_c[1]+t_c[2])
    calcu_.append(t_)
    

# fixed bin size
bins = np.arange(0, 1.5, 0.02) # fixed bin size
plt.xlim([0, 1.2])

#plt.hist(data, bins=bins, alpha=0.5)
plt.hist(calcu_, bins=bins, alpha=0.5)
...
#calculate the percentage of pairs with good spectrums
count_0_5 = int(0)
count_1 = int(0)
count_2 = int(0)
count_3 = int(0)
for i in range(len(calcu_)):
    if calcu_[i] <=0.05:
        count_0_5 += 1
    if calcu_[i] <=0.1:
        count_1 += 1
...
percent_0_5 = float(count_0_5)/float(len(calcu_))
percent_1 = float(count_1)/float(len(calcu_))
percent_2 = float(count_2)/float(len(calcu_))
percent_3 = float(count_3)/float(len(calcu_))
print('good spectrum pairs in SimLex-999', percent_0_5, percent_1, percent_2, percent_3)  



#####################################################
####Plot the spectrums of some examples##############
#####################################################
pl_0 = []
with open("example", 'r') as f_0:
    for line in f_0:
        c = map(float, line.split())
        pl_0.append(c[0])
        pl_0.append(c[1])
        pl_0.append(c[2])
        pl_0.append(c[3])
...
min__ = min(min_list)
max__ = max(max_list)
axs[0,0].set_ylim([min__, max__])
axs[0,1].set_ylim([min__, max__])
axs[1,0].set_ylim([min__, max__])
axs[1,1].set_ylim([min__, max__])

axs[0,0].bar(x,pl_0)
axs[0,0].title.set_text('trade.n.01')
axs[0,1].bar(x,pl_1)
axs[0,1].title.set_text('fair_trade.n.02')
axs[1,0].bar(x,pl_2)
axs[1,0].title.set_text('fair_trade.n.01')
axs[1,1].bar(x,pl_3)
axs[1,1].title.set_text('free_trade.n.01')
plt.tight_layout(rect=[0, 0, 1, 0.95])
fig.savefig("ACL_paper/"+"n"+".pdf", bbox_inches='tight')
